...
| Context-zone emoji | `🤓 <80k → 🤯 <100k → 😵‍💫 <110k → 💀` on context tokens |
...
- **Context tokens** — green → yellow → amber → red across the same `80k / 100k / 110k` zone thresholds as the emoji.
...
Emoji, thresholds, and the segment delimiter are read from `statusline.config.json`, resolved next
to the script (`~/.claude/statusline.config.json` once installed). Every key is optional — any
omitted key falls back to the in-script default, so the script still renders with no config at all.
...
| `smart_zone.token_count.{smart,good,poor}` | context-token thresholds for the emoji and colour ladder |
| `smart_zone.emoji.{smart,good,poor,dumb}` | the four context-zone emoji |
| `usage.pct_{green,yellow,amber}_max` | upper bound of each usage-percentage colour band |
| `usage.emoji.{session,weekly}` | the 5h / 7d widget emoji |
