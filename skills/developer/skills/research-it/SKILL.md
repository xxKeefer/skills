...
Investigate options and produce a research artifact before committing to a spike. The output is a
decision document that pre-steers `/spike-it` sessions and gives fast context to any agent or human
picking up the work later.
...
- **Tracker ticket** — a reference in the project's issue tracker (a GitHub issue, a Jira key, etc.,
  per the repo's CLAUDE.md). Fetch its context via `/look-up`
...
Use web search for current ecosystem data. Use codebase exploration to understand existing patterns
and constraints.

### Anti-approaches

Equally important: document approaches that were considered and **rejected**, with clear reasoning.
This prevents future agents or team members from re-investigating dead ends.
...
Use `/write-to-file` to write `research_{terse-description}.md` to the `.ai/` directory.
...
- **Stale research is worse than none** — a research doc that drifts from reality actively misleads.
  Every time a research artifact is referenced, verify its claims still hold. If a decision was
  overturned, a library was swapped, or an approach failed in practice — update or delete the
  artifact immediately. Never leave stale research lying around.
- **Decisions, not descriptions** — the artifact captures what was decided and why, not a survey of
...
- **Durable** — no assumptions about who reads this next. An agent dropped into the repo cold should
  get full context from this document alone
- **Prune aggressively** — once a research artifact has been fully consumed by a spike and the
  decisions are implemented, delete it. The spike and code are now the source of truth. Research
  docs are transient by nature — they exist to inform decisions, not to document them permanently
